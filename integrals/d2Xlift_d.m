...
d2X_pen(2:K  , :) = d2X_pen(2:K  , :)-2.*fKl(2:K  , :).*Xjmpl./g.detJ0T(2:K  , 1); % pen (l)
d2X_pen(1:K-1, :) = d2X_pen(1:K-1, :)+2.*fKr(1:K-1, :).*Xjmpr./g.detJ0T(1:K-1, 1); % pen (r)
switch eq
    case 1
    ...
    case 2
        ...
        d2X_pen(1, :) = d2X_pen(1, :)-2.*fKl(1, :).*Xl(1, 1)./g.detJ0T(1, 1); % pen (l)
        d2X_pen(K, :) = d2X_pen(K, :)+2.*fKr(K, :).*Xr(K, 1)./g.detJ0T(K, 1); % pen (r)
    otherwise
        return
end
%--------------------------------------------------------------------------
