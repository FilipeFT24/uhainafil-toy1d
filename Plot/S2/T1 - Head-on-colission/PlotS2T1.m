...
LW     = 2.5;
flag   = [0, 1, 0];
path11 = "Plot/S2/T1 - Head-on-colission/DATA";
path12 = "Plot/S2/T1 - Head-on-colission";
path13 = "Plot/S2/T1 - Head-on-colission/";
path21 = "Plot/S2/T2 - Wall/DATA";
path22 = "Plot/S2/T2 - Wall";
...
str2   = ["Hoc", "Wall"];
for i = 1:m
    Fh1C(:, :, i) = getdata1(sprintf("%s/0.%s/%s_P%d_2000.mat", path11, str1(1, i), str2(1, 1), p), p, 1);
    Fh2C(:, :, i) = getdata2(sprintf("%s/0.%s/%s_P%d_2000.mat", path11, str1(1, i), str2(1, 1), p), p, 1, ns);
    th1C(:, i)    = getdata3(sprintf("%s/0.%s/%s_P%d_2000.mat", path11, str1(1, i), str2(1, 1), p));
    Fh1W(:, :, i) = getdata1(sprintf("%s/0.%s/%s_P%d_1000.mat", path21, str1(1, i), str2(1, 2), p), p, 1);
    Fh2W(:, :, i) = getdata2(sprintf("%s/0.%s/%s_P%d_1000.mat", path21, str1(1, i), str2(1, 2), p), p, 1, ns);
    th1W(:, i)    = getdata3(sprintf("%s/0.%s/%s_P%d_1000.mat", path21, str1(1, i), str2(1, 2), p));
end
Fh1C               = Fh1C-h0;
Fh1W               = Fh1W-h0;
Fh2C(:, 2:K1+1, :) = Fh2C(:, 2:K1+1, :)-h0;
Fh2W(:, 2:K2+1, :) = Fh2W(:, 2:K2+1, :)-h0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if flag(1, 1)
    ...
    P11           = cell(2, m);
    P12           = cell(m, 3);
    fid11         = ["Hoc_P3.pdf"; "Wall_P3.pdf"];
    fid12         = [sprintf("HocWall1_P3 (0.%s).pdf", str1(1, 1)); sprintf("HocWall1_P3 (0.%s).pdf", str1(1, 2))];
    %----------------------------------------------------------------------
    ylim10        = [-0.05, 1.15];
    ytick10       = [-0.05, 0.00:0.25:1.00, 1.15];
    yticklabel10  = ["-0.05", "0.00", "0.25", "0.50", "0.75", "1.00", "1.15"];
    ylim11        = [-0.02, 0.50];
    ytick11       = [-0.02, 0.00:0.10:0.50];
    yticklabel11  = ["-0.02", "0.00", "0.10", "0.20", "0.30", "0.40", "0.50"];
    ylim12        = [-0.15, 3.00];
    ytick12       = [-0.15, 0.00:0.50:3.00];
    yticklabel12  = ["-0.15", "0.00", "0.50", "1.00", "1.50", "2.00", "2.50", "3.00"];
    %----------------------------------------------------------------------
    ylim1         = {ylim11; ylim12};
    ytick1        = {ytick11; ytick12};
    yticklabel1   = {yticklabel11; yticklabel12};
    %----------------------------------------------------------------------
    legendlabel11 = ["$\epsilon = 0.21$", "$\epsilon = 0.96$"];
    legendlabel12 = ["$t^{\prime} = t_{0}^{\prime}$", "$t^{\prime} = t_{f}^{\prime}$", "$t^{\prime} = t_{p}^{\prime}$"];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i = 1:2
        %------------------------------------------------------------------
        if ~export(1, 1)
            fig11 = figure('Color', 'w', 'Windowstate', 'maximized');
        else
            fig11 = figure('Color', 'w', 'Windowstate', 'maximized', 'MenuBar', 'none', 'Toolbar', 'none');
        end
        hold on;
        %------------------------------------------------------------------
        switch i
    ...
    for i = 1:m
        %------------------------------------------------------------------
        if ~export(1, 1)
            fig12 = figure('Color', 'w', 'Windowstate', 'maximized');
        else
            fig12 = figure('Color', 'w', 'Windowstate', 'maximized', 'MenuBar', 'none', 'Toolbar', 'none');
        end
        hold on;
        %------------------------------------------------------------------
        patch(...
...
if flag(1, 2)
    ...
    %----------------------------------------------------------------------
    ylim21       = [-0.02, 0.50];
    ytick21      = [-0.02, 0.00:0.10:0.50];
    yticklabel21 = ["-0.02", "0.00", "0.10", "0.20", "0.30", "0.40", "0.50"];
    ylim22       = [-0.50, 2.65];
    ytick22      = [-0.50:0.50:2.50, 2.65];
    yticklabel22 = ["-0.50", "0.00", "0.50", "1.00", "1.50", "2.00", "2.50", "2.65"];
    %----------------------------------------------------------------------
    ylim2        = {ylim21; ylim22};
    ...
    for i = 2%1:2
        for j = 2%1:m
            %--------------------------------------------------------------
            if ~export(1, 2)
                fig2 = figure('Color', 'w', 'Windowstate', 'maximized');
            else
                fig2 = figure('Color', 'w', 'Windowstate', 'maximized', 'MenuBar', 'none', 'Toolbar', 'none');
            end
            hold on;
            %--------------------------------------------------------------
            switch i
...
if flag(1, 3)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    P4           = cell(2, 4);
    fid3         = ["Hoc_P3 (xp).pdf"; "Wall_P3 (tp).pdf"];
    %----------------------------------------------------------------------
    Za           = {...
        za(1, 1, 0.21);
        za(1, 1, 0.96)};
    ...
    ylim32       = [0.15, 0.50];
    ytick32      = [0.15, 0.20:0.10:0.50];
    yticklabel32 = ["0.15", "0.20", "0.30", "0.40", "0.50"];
    ylim33       = [0.50, 2.65];
    ytick33      = [0.50, 0.65, 1.00:0.50:2.50, 2.65];
    yticklabel33 = ["0.50", "0.65", "1.00", "1.50", "2.00", "2.50", "2.65"];
    %----------------------------------------------------------------------
    xtitle3      = "$t^{\prime}$";
    ytitle31     = "$x_{p}$";
    ytitle32     = "$\zeta_{p}^{\prime}$";
    legendlabel3 = ["$\epsilon = 0.21$", "$\epsilon = 0.96$", "$\textrm{Analytical}$", "$k = 3$"];
    numcols3     = 1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i = 1:2
        %------------------------------------------------------------------
        if ~export(1, 1)
            fig3 = figure('Color', 'w', 'Windowstate', 'maximized');
        else
            fig3 = figure('Color', 'w', 'Windowstate', 'maximized', 'MenuBar', 'none', 'Toolbar', 'none');
        end
        hold on;
        %------------------------------------------------------------------
        for j = 1:m
