...
%--------------------------------------------------------------------------
if any(log_wd, 1)
    if Z_l(wet2, 1) < Z_r(wet1-1, 1)+eps
        Z_1 = Z_r(wet1-1, 1); % lake at rest
    else
        Z_1 = Zbl(dry2  , 1); % run-up/down
    end
    Hu1 = Hum(wet1, 1);
    Zb1 =-H_m(wet1, 1)+Z_1;
    %----------------------------------------------------------------------
    ...
    %----------------------------------------------------------------------
    if Z_l(dry1, 1) < Z_1
        g.x  (dry1, :, 1) = repmat(Z_1, 1, N);
        g.x  (dry1, :, 2) = 0;
        g.zb (dry1, :)    = repmat(Z_1, 1, N);
        g.fix(dry1, 1)    = true;
    end
    %----------------------------------------------------------------------
end
...
if any(log_dw, 1)
    %----------------------------------------------------------------------
    if Z_r(wet2, 1) < Z_l(wet2+1, 1)+eps
        Z_2 = Z_l(wet2+1, 1); % lake at rest
    else
        Z_2 = Zbr(dry2  , 1); % run-up/down
    end
    Zb2 =-H_m(wet2, 1)+Z_2;
    Hu2 = Hum(wet2, 1);
    %----------------------------------------------------------------------
    ...
    %----------------------------------------------------------------------
    if Z_r(dry2, 1) < Z_2
        g.x  (dry2, :, 1) = repmat(Z_2, 1, N);
        g.x  (dry2, :, 2) = 0;
        g.zb (dry2, :)    = repmat(Z_2, 1, N);
        g.fix(dry2, 1)    = true;
    end
    %----------------------------------------------------------------------
end
